...
namespace SyncCto\Contao\Finder;
...
use SyncCto\Contao\Automator\Automator;
use SyncCto\Contao\Controller\Client;
use SyncCto\Filter\FilterIteratorBase;
use SyncCto\Filter\FilterIteratorFiles;
use SyncCto\Filter\FilterIteratorFolder;
use SyncCto\Helper\Helper;

/**
 * Class for file operations
 */
class Finder extends Backend
{
    ////////////////////////////////////////////////////////////////////////////
    // Vars
    ////////////////////////////////////////////////////////////////////////////
    ...
    protected function generateChecksumFolders($booCore = false, $booFiles = false)
    {
        ...
        // Check each file
        foreach ((array) $this->getFolderList($booCore, $booFiles) as $objFolder)
        {
...
//                    }
    ...
    public function getFileList($booRoot = false, $booFiles = false)
    {
        ...
        if ($booRoot == true)
        {
            ...
            $objFilterIt     = new FilterIteratorFiles($objDirectoryIt);
            $objRecursiverIt = new \RecursiveIteratorIterator($objFilterIt, \RecursiveIteratorIterator::SELF_FIRST);
    ...
    public function getFolderList($booRoot = false, $booFiles = false)
    {
        ...
        if ($booRoot == true)
        {
            foreach ($this->arrRootFolderList as $value)
            {
                ...
                // Scann.
                $objDirectoryIt  = new \RecursiveDirectoryIterator($strFullPath, $this->strRDIFlags);
                $objFilterIt     = new FilterIteratorFolder($objDirectoryIt);
                $objRecursiverIt = new \RecursiveIteratorIterator($objFilterIt, \RecursiveIteratorIterator::SELF_FIRST);
    ...
    public function moveTempFile($arrFileList, $blnIsDbafs)
    {
        foreach ($arrFileList as $key => $value)
        {
            try
            {
                ...
                {
                    $arrFileList[ $key ]['saved']        = true;
                    $arrFileList[ $key ]['transmission'] = Enum::FILETRANS_MOVED;
                }
                else
                {
                    $arrFileList[$key]['saved']        = false;
