...
class SyncCtoFiles extends Backend
{
    ...
    public function __construct()
    {
        ...
        $this->strRDIFlags = FilesystemIterator::FOLLOW_SYMLINKS | FilesystemIterator::SKIP_DOTS | FilesystemIterator::UNIX_PATHS;
    }
    ...
    public function getDbafsInformation($strFilePath, $blnAutoAdd = false, $blnSkipExistsCheck = false)
    {
        ...
        if ($objModel == null && $blnAutoAdd) {
            ...
            return $arrModelData;
        } elseif ($objModel == null && !$blnAutoAdd) { // If empty and auto add disable retrun null for this file.
            return null;
        } else { // If we have data add it to the file
            $arrModelData = $objModel->row();
            $arrModelData['uuid'] = StringUtil::binToUuid($arrModelData['uuid']);
            $arrModelData['pid'] = (strlen($arrModelData['pid'])) ? StringUtil::binToUuid($arrModelData['pid']) : $arrModelData['pid'];
            //$arrModelData['tail'] = $this->getDbafsTailFor($arrModelData['pid']);
    ...
    public function updateDbafs($arrFileList)
    {
        foreach ($arrFileList as $key => $arrFile) {
            ...
            if ($arrFile['dbafs_state'] == SyncCtoEnum::DBAFS_DATA_CONFLICT) {
                ...
                $intFileNumber = 1;
                for ($i = 1; $i < 100; $i++) {
                    $strNewDestinationName = sprintf(
                        '%s' . DIRECTORY_SEPARATOR . '%s_%s.%s',
                    ...
                    if (!file_exists(SyncCtoHelper::getInstance()->getContaoRoot() . DIRECTORY_SEPARATOR . $strNewDestinationName)) {
                        $intFileNumber = $i;
                        break;
                    }
                }
        ...
        foreach ($this->getFileList($booCore, $booFiles) as $objFile) {
            ...
            if ($intSize < 0 && $intSize != 0) {
                ...
                if ($intSize >= $GLOBALS['SYC_SIZE']['limit_ignore']) {
                    ...
                    if ($intSize >= $GLOBALS['SYC_SIZE']['limit']) {
                        $arrChecksum[md5($strRelativePath)] = array(
                            "path"         => $strRelativePath,
    ...
    public function generateChecksumFileAsXML($strXMLFile, $booCore = false, $booFiles = false, $intInformations = SyncCtoEnum::FILEINFORMATION_SMALL)
    {
        ...
        foreach ($objFileIterator as $objFile) {
            ...
            if ($intSize < 0 && $intSize != 0) {
                ...
                if ($intInformations == SyncCtoEnum::FILEINFORMATION_SMALL) {
                    ...
                    if ($intInformations == SyncCtoEnum::FILEINFORMATION_BIG) {
                        $objXml->startElement('file');
                        $objXml->writeAttribute("id", md5($strRelativePath));
                        $objXml->writeAttribute("ai", $i);
                        $objXml->text($strRelativePath);
        ...
        if (file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strXMLFile)) {
            return true;
        } else {
            return false;
        }
    ...
    public function runCecksumCompare($arrChecksumList, $blnDisableDbafsConflicts = false)
    {
        ...
        foreach ($arrChecksumList as $key => $value) {
            ...
            if ($value['state'] == SyncCtoEnum::FILESTATE_BOMBASTIC_BIG) {
                ...
                if (file_exists($this->objSyncCtoHelper->getFullPath($value['path']))) {
...
//                    }
    ...
    public function runDump($strZip = "", $booCore = false, $arrFiles = array())
    {
        if ($strZip == "") {
            ...
            $strFilename = StringUtil::standardize(str_replace(array(" "), array("_"), preg_replace("/\.zip\z/i", "", $strZip))) . ".zip";
        }
        ...
        if (($mixError = $objZipArchive->open($strPath, ZipArchive::CREATE)) !== true) {
        ...
        foreach ((array) $arrFiles as $file) {
            ...
            if (file_exists($strFullFilePath) && is_dir($strFullFilePath)) {
                ...
                if (file_exists($strFullFilePath)) {
    ...
    public function runIncrementalDump($srtXMLFilelist, $strZipFolder, $strZipFile = null, $intMaxFilesPerRun = 5)
    {
        ...
        if (!file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $srtXMLFilelist)) {
        ...
        $objXml->load($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $srtXMLFilelist);
        ...
        // Open ZipArchive
        $objZipArchive = new ZipArchiveCto();
        if (($mixError = $objZipArchive->open($strZipPath, ZipArchive::CREATE)) !== true) {
        ...
        foreach ($objFilesList as $file) {
            ...
            if (file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $file->nodeValue)) {
            ...
            if ($intRuns == $intMaxFilesPerRun) {
                $objZipArchive->close();
                $objZipArchive->open($strZipPath, ZipArchive::CREATE);
                $intRuns = 0;
            }
        ...
        // Close XML and zip
        $objXml->save($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $srtXMLFilelist);
        $objZipArchive->close();
    ...
    public function getFileListFromFolders($arrFolders = array())
    {
        ...
        foreach ($arrFolders as $strFolder) {
            // Scann.
            $objDirectoryIt = new RecursiveDirectoryIterator($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $this->objSyncCtoHelper->standardizePath($strFolder), $this->strRDIFlags);
            $objFilterIt = new SyncCtoFilterIteratorBase($objDirectoryIt);
            $objRecursiverIt = new RecursiveIteratorIterator($objFilterIt, RecursiveIteratorIterator::SELF_FIRST);
    ...
    public function getFileList($booRoot = false, $booFiles = false)
    {
        ...
        if ($booRoot == true) {
            ...
            $objDirectoryIt = new RecursiveDirectoryIterator($this->objSyncCtoHelper->getContaoRoot(), $this->strRDIFlags);
            $objFilterIt = new SyncCtoFilterIteratorFiles($objDirectoryIt);
            $objRecursiverIt = new RecursiveIteratorIterator($objFilterIt, RecursiveIteratorIterator::SELF_FIRST);

            $objAppendIt->append($objRecursiverIt);

            // Scan allowed root folders.
            foreach ($this->arrRootFolderList as $value) {
                $strFullPath = $this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $this->objSyncCtoHelper->standardizePath($value);
    ...
    public function splitFiles($strSrcFile, $strDesFolder, $strDesFile, $intSizeLimit)
    {
        ...
        if (!file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strSrcFile)) {
        ...
        $i = 0;
        for ($i; $booRun; $i++) {
            $fp = fopen($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strSrcFile, "rb");
    ...
    public function moveTempFile($arrFileList, $blnIsDbafs)
    {
        foreach ($arrFileList as $key => $value) {
            try {
                ...
                if (!file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strTempFile)) {
                    $arrFileList[$key]['saved'] = false;
                    ...
                    if ($objLocaleData == null) {
                        ...
                        elseif ($strLocaleUUID != $value['tl_files']['uuid']) {
                            ...
                            for ($i = 1; $i < 100; $i++) {
                                ...
                                if (!file_exists($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strNewDestinationName)) {
                                    $intFileNumber = $i;
                                    break;
                                }
                            }
    ...
    public function saveFiles($arrMetafiles)
    {
        ...
        foreach ($_FILES as $key => $value) {
            ...
            if ($this->objFiles->move_uploaded_file($value["tmp_name"], $strSaveFile) === FALSE) {
                ...
                if ($key != md5_file($this->objSyncCtoHelper->getContaoRoot() . DIRECTORY_SEPARATOR . $strSaveFile)) {
                    throw new Exception($GLOBALS['TL_LANG']['ERR']['checksum_error']);
                } else {
