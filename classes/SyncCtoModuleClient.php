...
class SyncCtoModuleClient extends \BackendModule
{
    ...
    protected function compile()
    {
        ...
        try {
            ...
            throw $exc;
            $_SESSION["TL_ERROR"] = [ $GLOBALS['TL_LANG']['ERR']['client_set'] ];
    ...
    private function pageSyncTo()
    {
        ...
        if ($this->intStep == 0) {
            ...
            \Session::getInstance()->set("SyncCto_FileLock_ID" . $this->intClientID, [ "lock" => false ]);
        }
            ...
            switch ($this->objStepPool->step) {
                ...
                case 4:
                    ...
                    if ($strVersion != $strCurrentVersion) {
                        $this->log(vsprintf("Not the same version from contao on synchronization client ID %s. Serverversion: %s. Clientversion: %s", [ \Input::get("id"), $GLOBALS['SYC_VERSION'], $strVersion ]), __CLASS__ . " " . __FUNCTION__, "GENERAL");

                        $this->objData->setState(SyncCtoEnum::WORK_ERROR);
                        $this->booError = true;
                        $this->strError = vsprintf($GLOBALS['TL_LANG']['ERR']['version'], [ "Contao", $strCurrentVersion, $strVersion ]);
                        break;
                    }
            ...
            switch ($this->objStepPool->step) {
                ...
                case 2:
                    ...
                    if ($isAdmin || !empty($userTables)) {
                        ...
                        $this->objStepPool->step++;
                    } else if (empty($userTables) && ($this->arrSyncSettings['syncCto_SyncTlFiles'] || $this->arrSyncSettings['automode'])) {
                        $this->arrSyncSettings['syncCto_SyncTables']['tl_files'] = 'tl_files';
        ...
        try {
            switch ($this->objStepPool->step) {
                ...
                case 6:
                    ...
                    else if (is_array($this->arrListCompare) && ((array) count((array)$this->arrListCompare['core']) != 0 || count((array) $this->arrListCompare['files']) != 0)) {
                        $this->objData->setHtml("");
                        $this->objData->setState(SyncCtoEnum::WORK_OK);
            ...
            switch ($this->objStepPool->step) {
                ...
                case 11:
                    ...
                    if ($intCountMissing == 0 && $intCountNeed == 0 && $intCountIgnored == 0 && $intCountDelete == 0) {
                        ...
                        break;
                    } else if (count((array)$this->arrListCompare) == 0 || array_key_exists("skip", $_POST)) {
                        $this->objData->setState(SyncCtoEnum::WORK_SKIPPED);
                        $this->objData->setDescription($GLOBALS['TL_LANG']['tl_syncCto_sync']["step_2"]['description_1']);
                        $this->objData->setHtml("");
                        $this->booRefresh = true;
                        ...
                        break;
                    } else if (($this->arrSyncSettings["automode"] || array_key_exists("forward", $_POST)) && count((array)$this->arrListCompare) != 0) {
                        $this->objData->setState(SyncCtoEnum::WORK_OK);
                        $this->objData->setDescription(vsprintf($GLOBALS['TL_LANG']['tl_syncCto_sync']["step_2"]['description_4'], [ $intCountMissing, $intCountNeed, $intCountDelete, $intCountIgnored, $this->getReadableSize($intTotalSizeNew), $this->getReadableSize($intTotalSizeChange), $this->getReadableSize($intTotalSizeDel) ]));
                        $this->objData->setHtml("");
                        $this->booRefresh = true;
            ...
            switch ($this->objStepPool->step) {
                ...
                case 3:
                    ...
                    if (($this->arrSyncSettings['automode'] || array_key_exists("forward", $_POST)) && !(count((array)$this->arrSyncSettings['syncCto_SyncTables']) == 0 && count((array)$this->arrSyncSettings['syncCto_SyncDeleteTables']) == 0)) {
                        ...
                        break;
                    } else if (($this->arrSyncSettings['automode'] || array_key_exists("forward", $_POST)) && count((array)$this->arrSyncSettings['syncCto_SyncTables']) == 0 && count((array)$this->arrSyncSettings['syncCto_SyncDeleteTables']) == 0) {
                        // Skip if no tables are selected
                        $this->objData->setState(SyncCtoEnum::WORK_SKIPPED);
                        $this->objData->setHtml("");
                        $this->booRefresh = true;
        ...
        try {
            switch ($this->objStepPool->step) {
                ...
                case 6:
                    ...
                    else if (is_array($this->arrListCompare) && (count((array)$this->arrListCompare['core']) != 0 || count((array)$this->arrListCompare['files']) != 0)) {
                        $this->objData->setHtml("");
                        $this->objData->setState(SyncCtoEnum::WORK_OK);
                        $this->objData->setDescription(vsprintf($GLOBALS['TL_LANG']['tl_syncCto_sync']["step_3"]['description_2'], [ $intSendCount, (count((array)$this->arrListCompare['core']) + count((array)$this->arrListCompare['files'])) ]));
                        $this->booFinished = true;
